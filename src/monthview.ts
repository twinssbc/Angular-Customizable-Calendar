import {Component, OnInit, OnChanges, Input, Output, EventEmitter, SimpleChanges, TemplateRef} from '@angular/core';
        ...
        <div>
            <table class="table table-bordered table-fixed monthview-datetable">
                ...
                <tr *ngFor="let row of [0,1,2,3,4,5]">
                    <td *ngFor="let col of [0,1,2,3,4,5,6]" tappable (click)="select(view.dates[row*7+col])"
                        [ngClass]="getHighlightClass(view.dates[row*7+col])">
                        <ng-template [ngTemplateOutlet]="monthviewDisplayEventTemplate"
                                     [ngTemplateOutletContext]="{view: view, row: row, col: col}">
                        </ng-template>
                    </td>
                </tr>
                </tbody>
